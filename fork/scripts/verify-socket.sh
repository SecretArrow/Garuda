...
PKG="${1:-com.motion.browser}"

echo "==> [1/4] Socket presence"
adb shell cat /proc/net/unix | grep -E "motion-devtools|webview_devtools_remote" || {
  echo "FAIL: no DevTools abstract socket found"; exit 1; }

echo "==> [2/4] App installed & running"
adb shell pm list packages | grep -q "motion" || { echo "FAIL: app not installed"; exit 1; }
adb shell am start -n "$PKG/com.motion.browser.MainActivity"
sleep 4
