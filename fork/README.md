# Garuda Fork Track — Brave Android dengan DevTools socket internal
...
> SAMA; ketika fork siap, `DevToolsLocator.FORK_SOCKET_NAME = "garuda-devtools"`
...
2. `mkdir ~/garuda && cd ~/garuda`
3. `fetch --nohooks android` (atau clone brave-core + `npm run init`)
4. `gclient sync -D --with_branch_heads` (hook Android)
5. `gn gen out/Garuda arm64` dengan `target_os="android"`, `target_cpu="arm64"`
6. Terapkan `fork/patches/garuda-devtools-socket.patch` ke tree
7. `autoninja -C out/Garuda arm64` → `chrome_public_apk` (atau target brave)

## 3. Patch inti (Fase 1): DevTools pada abstract unix socket

`fork/patches/garuda-devtools-socket.patch`:
- `content/browser/devtools/devtools_http_handler.cc` — menambah `kUseAbstractUnixSocket`
  + `net::UnixDomainServerSocket` pada nama `"@garuda-devtools"` (NAMESPACE_ABSTRACT)
...
- Nama socket `@garuda-devtools` — `DevToolsLocator` di `:cdp-bridge` sudah
  memprioritaskannya.
...
# → 00000000: 00000002 00000000 00010000 0001 01 12345 @garuda-devtools
```

Lalu dari aplikasi Garuda (atau `:cdp-bridge` test): `DevToolsClient.autoDiscover()`
harus melaporkan socket `garuda-devtools` dan `listTargets()` mengembalikan tab.

## 5. Branding (Fase 0 item 3)

- `chrome/browser/ui/android/strings/...` → nama app "Garuda"
...
2. `DevToolsLocator.candidateNames()` sudah mengutamakan `garuda-devtools`.
