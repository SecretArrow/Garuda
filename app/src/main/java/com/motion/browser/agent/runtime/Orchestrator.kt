...
import com.motion.browser.agent.action.ApprovalGateway
...
import com.motion.browser.data.StepEntity
...
class AgentOrchestrator(
    ...
    inner class TaskApprovalGateway(private val taskId: String) : ApprovalGateway {
        override suspend fun requestApproval(taskId: String, question: String, detail: String): Boolean {
            ...
            notifier.notify("warn", "Motion Browser needs you", question)
        ...
        try {
            loop@ while (true) {
                ...
                val request = LlmRequest(
                    ...
                    messages = messages,
                    tools = com.motion.browser.agent.action.ToolSchemas.all,
                )
                ...
                if (call.name == "finish") {
                    ...
                    notifier.notify("info", "Motion Browser task done", result.summary.take(120))
                    break
                }
                if (!result.ok) {
